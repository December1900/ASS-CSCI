...
void TestNet2(float **x, float **d, int NumIPs, int NumOPs, int NumPats, int NumIts);

void TestNet3(float **x, float **d, int NumIPs, int NumOPs, int NumPats, int NumIts);

void TestNet4(float **x, float **d, int NumIPs, int NumOPs, int NumPats, int NumIts);
...
int main() {
    ...
    if (NumHN == 1) {
        ...
        TestNet2(IPTstData, OPTstData, NumIPs, NumOPs, NumTstPats, NumIts);
    }
    if (NumHN == 2) {
        TrainNet3(IPTrnData, OPTrnData, NumIPs, NumOPs, NumTrnPats, Ordering);
        TestNet3(IPTstData, OPTstData, NumIPs, NumOPs, NumTstPats, NumIts);
    }
    if (NumHN == 3) {
        TrainNet4(IPTrnData, OPTrnData, NumIPs, NumOPs, NumTrnPats, Ordering);
        TestNet4(IPTstData, OPTstData, NumIPs, NumOPs, NumTstPats, NumIts);
    }
...
void random_matrix(float **x, float **d, int NumIPs, int NumOPs, int p) {
    srand((unsigned) time(NULL));
    for (int j = 0; j < NumIPs; j++) {
        swap(x[p][j], x[rand() % (p + 1)][j]);
    }
    for (int j = 0; j < NumOPs; j++) {
        swap(d[p][j], d[rand() % (p + 1)][j]);
    }
}
...
void shuffleArrary(float **x, float **d, int NumIPs, int p) {
    srand((unsigned) time(NULL));
    int j = rand_X(NumIPs);
    swap(x[p][j], x[rand() % (p + 1)][j]);
    swap(d[p][j], d[rand() % (p + 1)][j]);
...
void TrainNet2(float **x, float **d, int NumIPs, int NumOPs, int NumPats, int Ordering) {
    ...
    cout << "Params: lrn_rat:" << LrnRate << " Order:" << Ordering << endl;
    ...
    for (;;) {// Main learning loop
        ...
        for (p = 0; p < NumPats; p++) {
            if (Ordering == 0) {
                ...
                random_matrix(x, d, NumIPs, NumOPs, p);
                for (i = 0; i < NumHN1; i++) { // Cal O/P of hidden layer 1
                    float in = 0;
                    for (j = 0; j < NumIPs; j++)
                        in += w1[j][i] * x[p][j];
                ...
                for (int q = 0; q < 20; q++) {
                    ...
                    PatErr = 0.0;
                    for (i = 0; i < NumOPs; i++) {
                        float err = y[i] - d[q][i]; // actual-desired O/P
                        if (err > 0)PatErr += err; else PatErr -= err;
                        int Temp_Err = NumErr;
                        NumErr += ((y[i] < 0.5 && d[q][i] >= 0.5) ||
                                   (y[i] >= 0.5 && d[q][i] < 0.5));//added for binary classification problem
                        if (NumErr > Temp_Err) {
                            temp = q;
                            for (i = 0; i < NumHN1; i++) { // Cal O/P of hidden layer 1
                                float in = 0;
                            ...
                            for (i = 0; i < NumOPs; i++) {
                                ad2[i] = (d[temp][i] - y[i]) * y[i] * (1.0 - y[i]);
                                for (j = 0; j < NumHN1; j++) {
                                    w2[j][i] += LrnRate * h1[j] * ad2[i] +
                                                Mtm1 * (w2[j][i] - w22[j][i]) +
                                                Mtm2 * (w22[j][i] - w222[j][i]);
                ...
                for (int q = 0; q < 20; q++) {
                    ...
                    for (i = 0; i < NumOPs; i++) {
                        ...
                        if (NumErr > Temp_Err) {
                            ...
                            // Learn pattern with back propagation
                            for (i = 0; i < NumOPs; i++) { // Modify layer 4 wts
                                ad3[i] = (d[temp][i] - y[i]) * y[i] * (1.0 - y[i]);
                                for (j = 0; j < NumHN2; j++) {
                                    w3[j][i] += LrnRate * h2[j] * ad3[i] +
                                                Mtm1 * (w3[j][i] - w33[j][i]) +
                                                Mtm2 * (w33[j][i] - w333[j][i]);
...
void TrainNet4(float **x, float **d, int NumIPs, int NumOPs, int NumPats, int Ordering) {
    ...
    cout << "TrainNet4: IP:" << NumIPs << " H1:" << NumHN1 << " H2:" << NumHN2 << " H3:" << NumHN3 << " OP:"
         << NumOPs
         << endl;
    cout << "Params: lrn_rat:" << LrnRate << " Order:" << Ordering << endl;
                ...
                for (int q = 0; q < 20; q++) {
                    ...
                    for (i = 0; i < NumOPs; i++) {
                        ...
                        if (NumErr > Temp_Err) {
                            ...
                            for (i = 0; i < NumOPs; i++) {
                                ad4[i] = (d[temp][i] - y[i]) * y[i] * (1.0 - y[i]);
                                for (j = 0; j < NumHN3; j++) {
                                    w4[j][i] += LrnRate * h3[j] * ad4[i] +
                                                Mtm1 * (w4[j][i] - w44[j][i]) +
                                                Mtm2 * (w44[j][i] - w444[j][i]);
...
void TestNet2(float **x, float **d, int NumIPs, int NumOPs, int NumPats, int NumIts) {
    ...
    cout << "    MinErr      AveErr      MaxErr      %Correct" << endl;
    AveErr /= NumPats;
    float PcntErr = NumErr / float(NumPats) * 100.0;
    cout.setf(ios::fixed | ios::showpoint);
    cout << setprecision(6) << setw(12) << MinErr << setw(12) << AveErr << setw(12) << MaxErr << setw(12) << PcntErr
         << endl;
    ofstream outfile;
    outfile.open("data.txt", ios::app);
    outfile << NumIts << "  " << PcntErr << "\n";
    delete[] h1;
    delete[] y;
}
